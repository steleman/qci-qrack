...
public:
    ...
    bool GlobalPhaseCompare(QStabilizerPtr toCompare, real1_f error_tol = TRYDECOMPOSE_EPSILON)
    {
        ...
        real1 argDiff = static_cast<real1>(std::abs((std::arg(thisAmpEntry.amplitude) -
                                                     std::arg(toCompare->GetAmplitude(thisAmpEntry.permutation))) / (2 * PI_R1)));
        argDiff -= (real1)(size_t)argDiff;
        if (argDiff > (ONE_R1 / 2)) {
            argDiff -= ONE_R1;
        }
        if (FP_NORM_EPSILON >= std::abs(argDiff)) {
            return false;
        }
        return error_tol >= ApproxCompareHelper(toCompare, error_tol, true);
    }
