...
protected:
    ...
    std::unique_ptr<complex[]> GetQubitReducedDensityMatrix(bitLenInt qubit) {
      ...
      dMtrx[1] = x / complex((real1)2, ZERO_R1) - I_CMPLX *
                (y / complex((real1)2, ZERO_R1));
      dMtrx[2] = x / complex((real1)2, ZERO_R1) + I_CMPLX *
                (y / complex((real1)2, ZERO_R1));
      dMtrx[3] = (ONE_CMPLX + z) / complex((real1)2, ZERO_R1);
      if (shards[qubit]) {
        complex adj[4]{ std::conj(shards[qubit]->gate[0]),
                        std::conj(shards[qubit]->gate[2]),
                        std::conj(shards[qubit]->gate[1]),
                        std::conj(shards[qubit]->gate[3]) };
        complex out[4];
    ...
    template <typename F>
    void CheckShots(unsigned shots, bitCapInt m, real1_f partProb,
    ...
    void FlushCliffordFromBuffers() {
      for (size_t i = 0U; i < qubitCount; ++i) {
        ...
        const real1 angle =
          (real1)(FractionalRzAngleWithFlush(i, std::arg(shard->gate[3U] /
                                                         shard->gate[0U])) / 2);
        if ((2 * std::abs(angle) / PI_R1) <= FP_NORM_EPSILON) {
          shards[i] = NULL;
          continue;
        }
    ...
    QStabilizerHybridPtr RdmCloneHelper() {
      CombineAncillae();
      QStabilizerHybridPtr clone =
        std::dynamic_pointer_cast<QStabilizerHybrid>(Clone());
    ...
    real1_f ExpectationFactorized(bool isFloat,
      ...
      return isFloat ?
             RdmCloneHelper()->stabilizer->ExpectationFloatsFactorizedRdm(roundRz,
                                                                          bits,
                                                                          weights) :
             RdmCloneHelper()->stabilizer->ExpectationBitsFactorizedRdm(roundRz,
                                                                        bits,
                                                                        perms,
                                                                        offset);
    }
...
public:
    QStabilizerHybrid(const std::vector<QInterfaceEngine>& eng,
                      bitLenInt qBitCount,
                      bitCapInt initState = ZERO_BCI,
    ...
    QStabilizerHybrid(bitLenInt qBitCount, bitCapInt initState = ZERO_BCI,
                      qrack_rand_gen_ptr rgp = nullptr,
                      complex phaseFac = CMPLX_DEFAULT_ARG, bool doNorm = false,
                      ...
                      bitLenInt qubitThreshold = 0U,
                      real1_f separation_thresh = FP_NORM_EPSILON_F)
    : QStabilizerHybrid({ QINTERFACE_OPTIMAL_BASE }, qBitCount, initState,
                        rgp, phaseFac, doNorm, randomGlobalPhase,
