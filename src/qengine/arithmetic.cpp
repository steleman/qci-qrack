...
void QEngineCPU::CINC(bitCapInt toAdd, bitLenInt inOutStart, bitLenInt length, const std::vector<bitLenInt>& controls)
{
    CHECK_ZERO_SKIP();
...
void QEngineCPU::INCBCD(bitCapInt toAdd, bitLenInt inOutStart, bitLenInt length)
{
    ...
    const bitLenInt nibbleCount = length / 4U;
    if (nibbleCount * 4U != length) {
        throw std::invalid_argument("BCD word bit length must be a multiple of 4.");
    }

    bitCapIntOcl maxPow = intPowOcl(10U, nibbleCount);
    toAdd %= static_cast<bitCapInt>(maxPow);
    if (!toAdd) {
        return;
    }
    ...
    const bitCapIntOcl otherMask =
      (maxQPowerOcl - static_cast<uint64_t>(ONE_BCI)) ^ inOutMask;
    ...
    ParallelFunc fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
        ...
        if (isValid) {
            ...
            for (bitLenInt j = 0; j < nibbleCount; ++j) {
                ...
                outInt |= static_cast<bitCapIntOcl>(nibbles[j]) <<
                  static_cast<bitCapIntOcl>(j * 4UL * static_cast<bitCapIntOcl>(ONE_BCI));
            }
...
void QEngineCPU::INCDECBCDC(bitCapInt toMod, bitLenInt inOutStart, bitLenInt length, bitLenInt carryIndex)
{
    ...
    const bitCapIntOcl carryMask = pow2Ocl(carryIndex);
    const bitCapIntOcl otherMask =
      (maxQPowerOcl - static_cast<uint64_t>(ONE_BCI)) ^ (inOutMask | carryMask);
    ...
    par_for_skip(0, maxQPowerOcl, pow2Ocl(carryIndex),
                 static_cast<bitLenInt>(ONE_BCI),
        ...
        inOutInt >>= 4UL * static_cast<bitCapIntOcl>(ONE_BCI);
        ...
        if (isValid) {
            ...
            for (bitLenInt j = 0; j < nibbleCount; ++j) {
                ...
                outInt |= static_cast<bitCapIntOcl>(nibbles[j]) <<
                  (j * 4UL * static_cast<bitCapIntOcl>(ONE_BCI));
            }
...
bitCapInt QEngineCPU::IndexedLDA(bitLenInt indexStart, bitLenInt indexLength,
                                 bitLenInt valueStart,
                                 bitLenInt valueLength, const void* values,
                                 bool resetValue) {
    ...
    if (valueBytes == 1) {
        fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
            nStateVec->write(
                lcv | static_cast<bitCapIntOcl>(reinterpret_cast<const uint8_t*>(values)[(lcv & inputMask) >> indexStart] << valueStart), stateVec->read(lcv));
        };
    } else if (valueBytes == 2) {
        const uint16_t* inputIntPtr = reinterpret_cast<const uint16_t*>(values);
        fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
            nStateVec->write(
                lcv | ((bitCapIntOcl)inputIntPtr[(lcv & inputMask) >> indexStart] << valueStart), stateVec->read(lcv));
        };
    } else if (valueBytes == 4) {
        const uint32_t* inputIntPtr = reinterpret_cast<const uint32_t*>(values);
        fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
            nStateVec->write(
                lcv | ((bitCapIntOcl)inputIntPtr[(lcv & inputMask) >> indexStart] << valueStart), stateVec->read(lcv));
        };
        ...
        fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
            ...
            for (bitCapIntOcl j = 0; j < valueBytes; ++j) {
                outputInt |=
                  static_cast<bitCapIntOcl>(reinterpret_cast<const bitCapIntOcl*>(values)[inputInt * valueBytes + j] << (8U * j));
            }
...
bitCapInt QEngineCPU::IndexedADC(bitLenInt indexStart, bitLenInt indexLength,
    ...
    ParallelFunc fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
        ...
        bitCapIntOcl outputInt = 0;
        if (valueBytes == 1) {
            outputInt = reinterpret_cast<const uint8_t*>(values)[inputInt];
        } else if (valueBytes == 2) {
            outputInt = reinterpret_cast<const uint16_t*>(values)[inputInt];
        } else if (valueBytes == 4) {
            outputInt = reinterpret_cast<const uint32_t*>(values)[inputInt];
        } else {
            for (bitCapIntOcl j = 0; j < valueBytes; ++j) {
                outputInt |= static_cast<bitCapIntOcl>(reinterpret_cast<const bitCapIntOcl*>(values)[inputInt * valueBytes + j] << (8U * j));
            }
        }
...
bitCapInt QEngineCPU::IndexedSBC(bitLenInt indexStart, bitLenInt indexLength,
                                 bitLenInt valueStart,
                                 bitLenInt valueLength, bitLenInt carryIndex,
                                 const void* values)
{
    if (isBadBitRange(indexStart, indexLength, qubitCount)) {
        throw std::invalid_argument("QEngineCPU::IndexedLDA range is out-of-bounds!");
...
void QEngineCPU::Hash(bitLenInt start, bitLenInt length, const void* values) {
    if (isBadBitRange(start, length, qubitCount)) {
    ...
    ParallelFunc fn = [&](const bitCapIntOcl& lcv, const unsigned& cpu) {
        ...
        if (bytes == 1) {
            ...
            for (bitCapIntOcl j = 0; j < bytes; ++j) {
                outputInt |=
                  static_cast<bitCapIntOcl>(reinterpret_cast<const bitCapIntOcl*>(values)[inputInt * bytes + j] << (8U * j));
            }
        }
