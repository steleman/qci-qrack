...
real1_f QEngineCUDA::GetExpectation(bitLenInt valueStart, bitLenInt valueLength)
{
    ...
    for (bitCapIntOcl i = 0U; i < static_cast<bitCapIntOcl>(maxQPower); ++i) {
...
void QEngineCUDA::ArithmeticCall(OCLAPI api_call,
                                 const bitCapIntOcl (&bciArgs)[BCI_ARG_LEN],
                                 const void* values, bitCapIntOcl valuesPower) {
  CArithmeticCall(api_call, bciArgs, NULL, 0U, values, valuesPower);
}

void QEngineCUDA::CArithmeticCall(OCLAPI api_call,
                                  const bitCapIntOcl (&bciArgs)[BCI_ARG_LEN],
                                  bitCapIntOcl* controlPowers, bitLenInt controlLen,
                                  const void* values, bitCapIntOcl valuesPower) {
  CHECK_ZERO_SKIP();
  ...
  if (controlLen) {
    tryCuda("Failed to enqueue buffer copy", [&] {
            return cudaMemcpy(nStateBuffer.get(), stateBuffer.get(),
  ...
  if (values) {
    loadBuffer = MakeBuffer(CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                            sizeof(unsigned char) * valuesPower, (void*) values);
...
bitCapInt QEngineCUDA::IndexedLDA(bitLenInt indexStart, bitLenInt indexLength,
...
bitCapIntOcl QEngineCUDA::OpIndexed(OCLAPI api_call, bitCapIntOcl carryIn,
  ...
  const bitCapIntOcl otherMask =
    (maxQPowerOcl - 1U) & (~(inputMask | outputMask | carryMask));
...
bitCapInt QEngineCUDA::IndexedADC(bitLenInt indexStart, bitLenInt indexLength,
                                  bitLenInt valueStart,
                                  bitLenInt valueLength, bitLenInt carryIndex,
                                  const void* values) {
  return OpIndexed(OCL_API_INDEXEDADC, 0U, indexStart, indexLength, valueStart,
                   valueLength, carryIndex, values);
}

/** Subtract based on an indexed load from classical memory */
bitCapInt QEngineCUDA::IndexedSBC(bitLenInt indexStart, bitLenInt indexLength,
                                  bitLenInt valueStart, bitLenInt valueLength,
                                  bitLenInt carryIndex, const void* values) {
...
/** Set 8 bit register bits based on read from classical memory */
void QEngineCUDA::Hash(bitLenInt start, bitLenInt length, const void* values) {
  const bitLenInt bytes = (length + 7) / 8;
  const bitCapIntOcl inputMask = bitRegMaskOcl(start, length);
  const bitCapIntOcl bciArgs[BCI_ARG_LEN] { maxQPowerOcl, start, inputMask,
