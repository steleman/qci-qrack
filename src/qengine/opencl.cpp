...
#define DISPATCH_BLOCK_WRITE(waitVec, buff, offset, length, array) \
  tryOcl("Failed to write buffer", \
         [&] { return queue.enqueueWriteBuffer(buff, CL_TRUE, offset, \
                                               length, array, waitVec.get()); }); \
  wait_refs.clear();
...
#define DISPATCH_WRITE(waitVec, buff, size, array) \
  device_context->EmplaceEvent( \
         [&](cl::Event& event) { \
         tryOcl("Failed to enqueue buffer write", \
                [&] { return queue.enqueueWriteBuffer(buff, CL_FALSE, 0U, \
                                                      size, array, \
                                                      waitVec.get(), &event); }); \
                                                      });
...
#define DISPATCH_BLOCK_READ(waitVec, buff, offset, length, array) \
  tryOcl("Failed to read buffer", \
         [&] { return queue.enqueueReadBuffer(buff, CL_TRUE, offset, length, \
                                              array, waitVec.get()); }); \
  wait_refs.clear();
...
#define WAIT_REAL1_SUM(buff, size, array, sumPtr) \
  ...
  tryOcl("Failed to enqueue buffer read", \
         [&] { return queue.enqueueReadBuffer(buff, CL_TRUE, 0U, \
                                              sizeof(real1) * size, array.get(), \
                                              NULL, NULL); }); \
...
void QEngineOCL::ArithmeticCall(OCLAPI api_call,
                                const bitCapIntOcl (&bciArgs)[BCI_ARG_LEN],
                                const void* values,
                                bitCapIntOcl valuesPower) {
  CArithmeticCall(api_call, bciArgs, NULL, 0U, values, valuesPower);
}

void QEngineOCL::CArithmeticCall(OCLAPI api_call,
                                 const bitCapIntOcl (&bciArgs)[BCI_ARG_LEN],
                                 bitCapIntOcl* controlPowers,
                                 bitLenInt controlLen, const void* values,
                                 bitCapIntOcl valuesPower) {
  CHECK_ZERO_SKIP();
  ...
  if (controlLen) {
    controlBuffer = MakeBuffer(CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                               sizeof(bitCapIntOcl) * controlLen, controlPowers);
  }
  ...
  if (controlLen) {
    device_context->EmplaceEvent([&](cl::Event& event) {
           tryOcl("Failed to enqueue buffer copy", [&] {
                  return queue.enqueueCopyBuffer(*stateBuffer, *nStateBuffer,
                                                 0U, 0U,
                                                 sizeof(complex) * maxQPowerOcl,
                                                 waitVec.get(), &event);
                  });
           });
  } else {
  ...
  PoolItemPtr poolItem = GetFreePoolItem();
  DISPATCH_WRITE(waitVec, *(poolItem->ulongBuffer),
                 sizeof(bitCapIntOcl) * BCI_ARG_LEN, bciArgs);
  ...
  if (values) {
    loadBuffer = MakeBuffer(CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                            sizeof(unsigned char) * valuesPower,
...
bitCapInt QEngineOCL::IndexedLDA(bitLenInt indexStart, bitLenInt indexLength,
                                 bitLenInt valueStart, bitLenInt valueLength,
                                 const void* values, bool resetValue) {
  ...
  const bitCapIntOcl bciArgs[BCI_ARG_LEN] {
    (bitCapIntOcl)(maxQPowerOcl >> valueLength), indexStart, inputMask,
    valueStart, valueBytes, valueLength, 0U, 0U, 0U, 0U };

  ArithmeticCall(OCL_API_INDEXEDLDA, bciArgs, values,
                 pow2Ocl(indexLength) * valueBytes);
...
bitCapIntOcl QEngineOCL::OpIndexed(OCLAPI api_call, bitCapIntOcl carryIn,
                                   bitLenInt indexStart, bitLenInt indexLength,
                                   bitLenInt valueStart, bitLenInt valueLength,
  ...
  const bitCapIntOcl inputMask = bitRegMaskOcl(indexStart, indexLength);
  const bitCapIntOcl outputMask = bitRegMaskOcl(valueStart, valueLength);
  const bitCapIntOcl otherMask = (maxQPowerOcl - 1U) &
                                 (~(inputMask | outputMask | carryMask));
  const bitCapIntOcl bciArgs[BCI_ARG_LEN] { maxQPowerOcl >> 1U, indexStart,
                                            inputMask, valueStart, outputMask,
                                            otherMask, carryIn, carryMask,
                                            lengthPower, valueBytes };
...
bitCapInt QEngineOCL::IndexedADC(bitLenInt indexStart, bitLenInt indexLength,
                                 bitLenInt valueStart, bitLenInt valueLength,
                                 bitLenInt carryIndex, const void* values) {
  return OpIndexed(OCL_API_INDEXEDADC, 0U, indexStart, indexLength,
                   valueStart, valueLength, carryIndex, values);
}

/** Subtract based on an indexed load from classical memory */
bitCapInt QEngineOCL::IndexedSBC(bitLenInt indexStart, bitLenInt indexLength,
                                 bitLenInt valueStart, bitLenInt valueLength,
                                 bitLenInt carryIndex, const void* values) {
  return OpIndexed(OCL_API_INDEXEDSBC, 1, indexStart, indexLength,
                   valueStart, valueLength, carryIndex,
                   const_cast<void*>(values));
}

/** Set 8 bit register bits based on read from classical memory */
void QEngineOCL::Hash(bitLenInt start, bitLenInt length,
                      const void* values) {
  const bitLenInt bytes = (length + 7) / 8;
  const bitCapIntOcl inputMask = bitRegMaskOcl(start, length);
  const bitCapIntOcl bciArgs[BCI_ARG_LEN]{ maxQPowerOcl, start, inputMask, bytes,
                                           0U, 0U, 0U, 0U, 0U, 0U };
...
void QEngineOCL::PhaseFlipX(OCLAPI api_call, const bitCapIntOcl* bciArgs) {
  ...
  cl::Event writeArgsEvent;
  DISPATCH_TEMP_WRITE(waitVec, *(poolItem->ulongBuffer),
                      sizeof(bitCapIntOcl) * 5, bciArgs, writeArgsEvent);
