...
{
    const std::vector<bitLenInt> controls { control };
    MCPhase(controls, ONE_CMPLX, complex((real1)cos(radians / 2), (real1)sin(radians / 2)), target);
}
...
void QInterface::CRX(real1_f radians, bitLenInt control, bitLenInt target)
{
    const real1 cosine = (real1)cos(radians / 2);
    const real1 sine = (real1)sin(radians / 2);
    const complex pauliRX[4]{ complex(cosine, ZERO_R1), complex(ZERO_R1, sine),
                              complex(ZERO_R1, sine), complex(cosine, ZERO_R1) };
    const std::vector<bitLenInt> controls { control };
    MCMtrx(controls, pauliRX, target);
}
#endif
} // namespace Qrack
