...
#define IS_REAL_1(r) (std::abs(ONE_R1 - r) <= FP_NORM_EPSILON)
...
QStabilizerHybrid::QStabilizerHybrid(const std::vector<QInterfaceEngine>& eng,
                                     bitLenInt qBitCount, bitCapInt initState,
                                     qrack_rand_gen_ptr rgp, complex phaseFac,
                                     bool doNorm, bool randomGlobalPhase,
                                     bool useHostMem, int64_t deviceId,
  ...
  shards(qubitCount) {
    const bitLenInt maxCpuQubitCount =
        getenv("QRACK_MAX_CPU_QB") ?
        static_cast<bitLenInt>(std::stoi(std::string(getenv("QRACK_MAX_CPU_QB"))))
        : 28U;
    const bitLenInt origMaxAncillaCount = maxAncillaCount;

#if ENABLE_OPENCL || ENABLE_CUDA
    const bool isQPager =
      (engineTypes[0U] == QINTERFACE_HYBRID) || (engineTypes[0U] == QINTERFACE_OPENCL);

    if (isQPager || ((engineTypes[0U] == QINTERFACE_QPAGER) &&
                     ((engineTypes.size() == 1U) ||
                      (engineTypes[1U] == QINTERFACE_OPENCL)))) {
      ...
      if (isQPager) {
        ...
        const bitLenInt perPage =
          log2Ocl(QRACK_GPU_SINGLETON.GetDeviceContextPtr(devID)->GetMaxAlloc() /
                  sizeof(complex)) - 1U;

#if ENABLE_OPENCL
        maxAncillaCount = (devCount < 2U) ?
                        (perPage + 3U) : (perPage + log2Ocl(devCount) + 2U);
#else
        ...
        if (getenv("QRACK_MAX_PAGE_QB")) {
          const bitLenInt maxPageSetting =
            static_cast<bitLenInt>(std::stoi(std::string(getenv("QRACK_MAX_PAGE_QB"))));
          ...
          maxEngineQubitCount =
            (maxEngineQubitCount > 1U) ? (maxEngineQubitCount - 1U): 1U;
        }
#endif
        if (getenv("QRACK_MAX_PAGING_QB")) {
          const bitLenInt maxPageSetting =
            static_cast<bitLenInt>(std::stoi(std::string(getenv("QRACK_MAX_PAGING_QB"))));
    ...
    maxStateMapCacheQubitCount =
      maxCpuQubitCount - ((QBCAPPOW < FPPOW) ? 1U : (1U + QBCAPPOW - FPPOW));
...
void QStabilizerHybrid::FlushIfBlocked(bitLenInt control, bitLenInt target, bool isPhase)
{
    ...
    if ((4 * std::abs(angle) / PI_R1) <= FP_NORM_EPSILON) {
        return;
    }
    ...
    if ((qubitCount + ancillaCount + deadAncillaCount) > maxEngineQubitCount) {
        ...
        while (i < maxQPower) {
            ...
            for (unsigned j = 0U; j < numCores; ++j) {
                futures.push_back(
                    std::async(std::launch::async, [j, p, &clones]() { return clones[j]->GetAmplitude(j + static_cast<uint32_t>(p)); }));
                bi_increment(&i, 1U);
            ...
            for (size_t j = 0U; j < futures.size(); ++j) {
                e->SetAmplitude(j + static_cast<uint32_t>(p), futures[j].get());
            }
        }
        clones.clear();
#else
        ...
        while (bi_compare(i, maxLcv) < 0) {
            ...
            for (unsigned j = 0U; j < numCores; ++j) {
                futures.push_back(std::async(std::launch::async, [j, p, qPower, &clones]() {
                    bitCapInt k = (j + static_cast<uint32_t>(p)) & (static_cast<uint32_t>(qPower) - 1U);
                    bi_or_ip(&k, ((j + static_cast<uint32_t>(p)) ^ static_cast<uint32_t>(k)) << 1U);
                    return std::norm(clones[j]->GetAmplitude(k | qPower));
                }));
                bi_increment(&i, 1U);
...
bitCapInt QStabilizerHybrid::MAll()
{
    ...
    while (i < maxQPower) {
        ...
        for (unsigned j = 0U; j < numCores; ++j) {
            futures.push_back(
                std::async(std::launch::async, [j, p, &clones]() { return std::norm(clones[j]->GetAmplitude(j + static_cast<uint32_t>(p))); }));
            bi_increment(&i, 1U);
            if (bi_compare(i, maxQPower) >= 0) {
                break;
            }
...
std::map<bitCapInt, int> QStabilizerHybrid::MultiShotMeasureMask(const std::vector<bitCapInt>& qPowers, unsigned shots)
{
    ...
    while (i < maxQPower) {
        ...
        for (unsigned j = 0U; j < numCores; ++j) {
            futures.push_back(
                std::async(std::launch::async, [j, p, &clones]() { return std::norm(clones[j]->GetAmplitude(p + j)); }));
            bi_increment(&i, 1U);
            if (bi_compare(i, maxQPower) >= 0) {
                break;
            }
        ...
        for (size_t j = 0U; j < futures.size(); ++j) {
            const real1 prob = futures[j].get();
            CHECK_SHOTS_IF_ANY(p + j, shotFunc);
        }
        if (!rng.size()) {
            break;
        }
...
void QStabilizerHybrid::CombineAncillae()
{
    ...
    for (size_t i = shards.size() - 1U; i >= qubitCount; --i) {
        ...
        if (std::abs(prob) <= FP_NORM_EPSILON) {
            stabilizer->H(i);
...
void QStabilizerHybrid::RdmCloneFlush(real1_f threshold)
{
    ...
    for (size_t i = shards.size() - 1U; i >= qubitCount; --i) {
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            if ((4 * std::abs(angle) / PI_R1) > threshold) {
...
{
    if ((nrm > ZERO_R1) && (std::abs(ONE_R1 - nrm) > FP_NORM_EPSILON)) {
        SwitchToEngine();
    }
