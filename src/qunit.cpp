...
bool QUnit::SeparateBit(bool value, bitLenInt qubit)
{
    ...
    if (!unit->isBinaryDecisionTree() && ((ONE_R1 / 2 - std::abs(prob)) > FP_NORM_EPSILON)) {
...
bool QUnit::INTSCOptimize(bitCapInt toMod, bitLenInt start, bitLenInt length,
                          bool isAdd, bitLenInt carryIndex,
    ...
    bitCapInt outInt;
    if (isAdd) {
        isOverflow = (overflowIndex != (bitLenInt)(-1)) &&
                      isOverflowAdd(inOutInt, inInt, signMask, lengthPower);
        outInt = static_cast<bitCapInt>(toMod) + inOutInt;
    } else {
        isOverflow = (overflowIndex != (bitLenInt)(-1)) &&
                      isOverflowSub(inOutInt, inInt, signMask, lengthPower);
        outInt = (inOutInt + lengthPower) - static_cast<uint64_t>(toMod);
    }

    const bool carryOut = bi_compare(outInt, lengthPower) >= 0;
    if (carryOut) {
        bi_and_ip(&outInt, lengthPower - static_cast<uint64_t>(ONE_BCI));
    }
...
bitCapInt
QUnit::GetIndexedEigenstate(bitLenInt indexStart, bitLenInt indexLength,
                            bitLenInt valueStart, bitLenInt valueLength,
                            const void* values) {
  const bitCapIntOcl indexInt =
    static_cast<bitCapIntOcl>(GetCachedPermutation(indexStart, indexLength));
  const bitLenInt valueBytes = (valueLength + 7U) / 8U;
  bitCapInt value = ZERO_BCI;
  const uint64_t* u64values = reinterpret_cast<const uint64_t*>(values);

  for (bitCapIntOcl j = 0U; j < valueBytes; ++j) {
    bi_or_ip(&value, u64values[indexInt * valueBytes + j] << (8U * j));
  }

  return value;
}

bitCapInt QUnit::GetIndexedEigenstate(bitLenInt start, bitLenInt length,
                                      const void* values) {
  ...
  for (bitCapIntOcl j = 0U; j < bytes; ++j) {
    bi_or_ip(&value, u64values[indexInt * bytes + j] << (8U * j));
  }

  return value;
}

bitCapInt QUnit::IndexedLDA(bitLenInt indexStart, bitLenInt indexLength,
                            bitLenInt valueStart, bitLenInt valueLength,
  ...
  if (isBadBitRange(valueStart, valueLength, qubitCount)) {
    ...
    // TODO: Index bits that have exactly 0 or 1 probability can be optimized
    // out of the gate.  This could follow the logic of UniformlyControlledSingleBit().
    // In the meantime, checking if all index bits are in eigenstates takes
    // very little overhead.
    if (CheckBitsPermutation(indexStart, indexLength)) {
      const bitCapInt value = GetIndexedEigenstate(indexStart, indexLength,
                                                   valueStart, valueLength, values);
    ...
    const bitCapInt toRet =
      std::dynamic_pointer_cast<QAlu>(shards[indexStart].unit)->IndexedLDA(
           shards[indexStart].mapped, indexLength, shards[valueStart].mapped,
...
bitCapInt QUnit::IndexedADC(bitLenInt indexStart, bitLenInt indexLength,
                            bitLenInt valueStart, bitLenInt valueLength,
...
bitCapInt QUnit::IndexedSBC(bitLenInt indexStart, bitLenInt indexLength,
                            bitLenInt valueStart, bitLenInt valueLength,
                            bitLenInt carryIndex, const void* values) {
  if (isBadBitRange(indexStart, indexLength, qubitCount)) {
...
#if ENABLE_VM6502Q_DEBUG
  if (CheckBitsPermutation(indexStart, indexLength) &&
      CheckBitsPermutation(valueStart, valueLength)) {
    bitCapInt value = GetIndexedEigenstate(indexStart, indexLength, valueStart,
                                           valueLength, values);
    value = GetCachedPermutation(valueStart, valueLength) - value;
    const bitCapInt valueMask = pow2Mask(valueLength);
    bool carry = false;
  ...
  const bitCapInt toRet =
    std::dynamic_pointer_cast<QAlu>(shards[indexStart].unit)->IndexedSBC(
         shards[indexStart].mapped, indexLength, shards[valueStart].mapped,
         valueLength, shards[carryIndex].mapped, values);
...
void QUnit::Hash(bitLenInt start, bitLenInt length, const void* values) {
  if (isBadBitRange(start, length, qubitCount)) {
  ...
  DirtyShardRange(start, length);
  std::dynamic_pointer_cast<QAlu>(EntangleRange(start, length))->Hash(
       shards[start].mapped, length, values);
}
