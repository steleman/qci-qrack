...
class qbad_alloc : public std::bad_alloc {
private:
  std::string w;

public:
  qbad_alloc() noexcept : std::bad_alloc() { }
  qbad_alloc(const qbad_alloc& rhs) noexcept : std::bad_alloc(rhs) { }
  qbad_alloc(const char* s) noexcept : std::bad_alloc(), w(s) { }
  qbad_alloc(const std::string& s) noexcept : std::bad_alloc(), w(s) { }
  ~qbad_alloc() = default;
  qbad_alloc& operator=(const qbad_alloc& rhs) {
    if (this != &rhs)
      w = rhs.w;
    ...
    for (size_t i = 0U; i < qinfos.size(); ++i) {
        ...
        if (sz) {
            ...
            for (size_t j = 0U; j < deviceList.size(); ++j) {
                ...
                const bitCapInt mqp = qinfos[i].unit->GetMaxQPower() + devSizes[j];
        ...
        if (bi_compare(deviceList[devIndex].maxSize,
                       qinfos[i].unit->GetMaxQPower() + devSizes[devIndex]) < 0) {
            throw qbad_alloc("QUnitMulti: device allocation limits exceeded.");
        }
